// https://www.nngroup.com/articles/cart-feedback/

'use client';

import { useCart } from '@/app/contexts/CartContext';
import { SquareOrderStatus } from '@/database/constants';
import { JSX, useEffect } from 'react';

type Params = {
    productName?: string, // /[productId] -> add to cart confirmation
    outOfStock?: SquareCartItem[], // /cart -> out of stock items
    orderInfo?: {
        status: string,
        redirect?: () => void,
    },
    orderError?: boolean,
    updateCartError?: boolean,
    loadShopError?: boolean,
    copyToClipboard?: JSX.Element, // /shop/checkout CreateOrder.tsx
}

// custom PageNote component appears on top of a page with a notice
// scroll down to view the different types of notices
export default function PageNote({ productName, outOfStock, orderInfo, orderError = false, updateCartError = false, loadShopError = false, copyToClipboard }: Params) {
    const { pageNoteState, closePageNotes, closeOutOfStock, closeOrderError, routerRefresh, closeCartError } = useCart();

    useEffect(() => {
        return ()=>{
            if(pageNoteState.toggle && productName || pageNoteState.updateCartError){
                closePageNotes();
            } else if(pageNoteState.toggle && pageNoteState.outOfStock.length > 0){
                closeOutOfStock();
            } else if(pageNoteState.toggle && pageNoteState.createOrderError){
                closeOrderError();
            } else if(pageNoteState.loadCartError){
                closeCartError();
            }
        }
        // eslint-disable-next-line react-hooks/exhaustive-deps
    }, [pageNoteState.toggle]);

    // confirmation notice: added to cart
    if(pageNoteState.toggle && productName && pageNoteState.productQuantity > 0){
        return(
            <div id="cart-confirmation"className="product-note-container">
                <div className="product-note">
                    <div className="title">
                        <i aria-hidden={true} className="fa fa-solid fa-circle-check"></i>
                        <span>{`Added ${pageNoteState.productQuantity}`} &times; {`${productName} to your cart.`}</span>
                    </div>
                </div>
            </div>
        );
    }
    // alert notice: failed to add item to cart
    else if(pageNoteState.toggle && productName && pageNoteState.addItemError){
        return(<div id="cart-error" className="product-note-container">
            <div className="product-note">
                <div className="title">
                    <i aria-hidden={true} className="fa fa-solid fa-triangle-exclamation"></i>
                    <span>Failed to add item</span>
                </div>
                <div className="body">
                    <p className="reason">There was an error adding this item to your cart. Please try again.</p>
                </div>
            </div>
        </div>);
    }
    // alert notice: failed to load shop products
    else if(loadShopError){
        return(<div id="cart-notice" className="product-note-container">
            <div className="product-note">
                <div className="title">
                    <i aria-hidden={true} className="fa fa-solid fa-triangle-exclamation"></i>
                    <span>Failed to load products</span>
                </div>
                <div className="body">
                    <p className="reason">There was an error loading products.</p>
                    <button
                        onClick={(e) => {
                            e.preventDefault();
                            routerRefresh();
                        }}
                        className="btn-secondary"
                    >
                        Try again
                    </button>
                </div>
            </div>
        </div>);
    }
    // alert notice: error loading cart
    else if(pageNoteState.loadCartError){
        return(<div id="cart-notice" className="product-note-container">
            <div className="product-note">
                <div className="title">
                    <i aria-hidden={true} className="fa fa-solid fa-triangle-exclamation"></i>
                    <span>Failed to load cart</span>
                </div>
                <div className="body">
                    <p className="reason">There was an error loading your cart.</p>
                    <button
                        onClick={(e) => {
                            e.preventDefault();
                            routerRefresh();
                        }}
                        className="btn-secondary"
                    >
                        Try again
                    </button>
                </div>
            </div>
        </div>);
    }
    // alert notice: failed to update cart
    else if(updateCartError){
        return(<div id="cart-notice" className="product-note-container">
            <div className="product-note">
                <div className="title">
                    <i aria-hidden={true} className="fa fa-solid fa-triangle-exclamation"></i>
                    <span>Failed to update cart</span>
                </div>
                <div className="body">
                    <p className="reason">There was an error updating your cart. Please try again.</p>
                </div>
            </div>
        </div>);
    }
    // alert notice: failed to create Square order
    else if(orderError){
        return(<div id="cart-notice" className="product-note-container">
            <div className="product-note">
                <div className="title">
                    <i aria-hidden={true} className="fa fa-solid fa-triangle-exclamation"></i>
                    <span>Failed to create order</span>
                </div>
                <div className="body">
                    <p className="reason">There was an error during checkout. Please try again.</p>
                </div>
            </div>
        </div>);
    }
    // alert notice: cart items out of stock
    // else if(pageNoteState.toggle && pageNoteState.outOfStock.length > 0){
    else if(outOfStock && outOfStock.length > 0){
        return(<div id="cart-notice" className="product-note-container">
            <div className="product-note">
                <div className="title">
                    <i aria-hidden={true} className="fa fa-solid fa-circle-exclamation"></i>
                    <span>Your cart has been updated</span>
                </div>
                <div className="body">
                    <p className="reason">These items are no longer available and have been removed from your cart.</p>
                    <ul>
                        {outOfStock.map((product, i) => (
                            <li key={i}>
                                <div className='cart-item'>
                                    <div className="cart-item-info">
                                        <div className="product">
                                            <div className="product-name">{product.squareItem.name}</div>
                                                <div className="product-modifiers">
                                                    {product.indexModifiers.length > 0 && product.indexModifiers.map((modindex: number, listindex: number) => (
                                                        <div key={listindex}>{product.squareItem.modifiers[listindex].list[modindex].name}</div>
                                                    ))}
                                                    {product.squareItem.variations.list[product.indexVariation].name}
                                                </div>
                                        </div>
                                        <div className="product-quantity-price">
                                            <div className="price">
                                                <span>{product.isArchived ? 'Unavailable' : 'Sold Out'}</span>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                            </li>
                        ))}
                    </ul>
                </div>
            </div>
        </div>);
    }
    // notice: order fulfillment
    else if(orderInfo){
        switch(orderInfo.status){
            case SquareOrderStatus.OPEN: // open -> fulfilled
                return (<div id="order-fulfilled"className="product-note-container">
                    <div className="product-note">
                        <div className="title">
                            <i aria-hidden={true} className="fa fa-solid fa-receipt"></i>
                            <span>Order status: Fulfilled</span>
                        </div>
                        <div className="body">
                            <p className="reason">Your order has been fulfilled with
                                <span className="square-logo">
                                    <span className="sr-only">Square</span>
                                    <svg aria-hidden={true} focusable={false} aria-label="Square Up" role="img" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 2000 501.43">
                                        <path fill="#1A1A1A" d="M501.43,83.79v333.84c0,46.27-37.5,83.79-83.79,83.79H83.79c-46.28,0-83.79-37.5-83.79-83.79V83.79C0,37.52,37.52,0,83.79,0h333.84c46.29,0,83.79,37.5,83.79,83.79ZM410.23,117.65c0-14.61-11.85-26.45-26.45-26.45H117.63c-14.61,0-26.45,11.84-26.45,26.45v266.19c0,14.61,11.84,26.45,26.45,26.45h266.17c14.61,0,26.45-11.85,26.45-26.45V117.65h-.02ZM182.32,197.6c0-8.43,6.79-15.26,15.17-15.26h106.4c8.39,0,15.17,6.84,15.17,15.26v106.24c0,8.43-6.75,15.26-15.17,15.26h-106.4c-8.39,0-15.17-6.84-15.17-15.26v-106.24ZM778.95,221.94l-3.85-.86c-41.04-9.31-65.81-14.93-65.81-42,0-24.2,23.02-41.11,55.98-41.11,30.52,0,53.74,12.76,73.08,40.16,1.11,1.57,2.84,2.61,4.74,2.84,1.89.23,3.79-.35,5.23-1.59l32.16-27.71c2.68-2.31,3.15-6.22,1.1-9.09-24.19-33.89-67.01-54.12-114.56-54.12-31.56,0-60.34,9.26-81.04,26.08-21.73,17.65-33.21,41.93-33.21,70.23,0,63.76,54.74,76.94,98.71,87.53,4.45,1.08,8.77,2.1,12.95,3.08,39.74,9.36,66,15.54,66,43.74s-24.04,45.48-61.24,45.48c-33.71,0-64.35-17.1-86.28-48.14-1.1-1.55-2.8-2.59-4.68-2.84-1.88-.25-3.73.28-5.2,1.49l-33.86,27.99c-2.72,2.25-3.28,6.14-1.3,9.05,25.63,37.64,76.48,61.97,129.56,61.97,32.56,0,62.52-9.57,84.36-26.95,23.27-18.51,35.57-44.01,35.57-73.73,0-67.27-57.62-80.13-108.45-91.48ZM1126.34,177.74h-40.76c-3.74,0-6.78,3.04-6.78,6.78v19.06c-12.6-14.21-33.77-30.22-65.18-30.22s-56.88,12.32-75.37,35.62c-17.16,21.63-26.62,51.73-26.62,84.75s9.45,63.12,26.62,84.75c18.49,23.31,44.56,35.62,75.37,35.62,26.63,0,49.1-9.45,65.18-27.37v107.92c0,3.74,3.04,6.78,6.78,6.78h40.76c3.74,0,6.78-3.04,6.78-6.78V184.52c0-3.74-3.04-6.78-6.78-6.78ZM1080.11,287.17v13.57c0,39.86-21.97,65.61-55.98,65.61-36.15,0-57.74-27.15-57.74-72.61s21.58-72.61,57.74-72.61c34.01,0,55.98,25.93,55.98,66.05ZM1360.6,177.74h-40.76c-3.74,0-6.78,3.04-6.78,6.78v130.66c0,32.45-23.32,49.42-46.36,49.42-26.03,0-39.79-15.58-39.79-45.04v-135.03c0-3.74-3.04-6.78-6.78-6.78h-40.76c-3.74,0-6.78,3.04-6.78,6.78v146.41c0,50.53,30.93,83.17,78.8,83.17,23.76,0,43.95-9.67,61.67-29.56v17.96c0,3.74,3.04,6.78,6.78,6.78h40.76c3.74,0,6.78-3.04,6.78-6.78v-217.99c0-3.74-3.04-6.78-6.78-6.78ZM1607.93,367.07c-8.79-.24-12.72-4.78-12.72-14.7v-98.9c0-51.66-31.71-80.11-89.3-80.11-44.71,0-80.07,23.24-94.59,62.18-.66,1.78-.52,3.78.39,5.47.93,1.73,2.57,2.98,4.48,3.42l37.83,8.71c3.37.77,6.78-1.14,7.94-4.45,6.74-19.11,20.01-28.01,41.76-28.01,25.53,0,38.48,11.62,38.48,34.54v3.2l-62.73,12.98c-53.04,11.03-77.74,34.25-77.74,73.09s32.22,68.73,78.36,68.73c28.25,0,51.2-8.69,66.46-25.16,9.13,17.73,33.93,26.29,62.3,21.35,3.24-.57,5.6-3.38,5.6-6.69v-28.9c0-3.63-2.93-6.67-6.52-6.77ZM1542.21,300.54v26.89c0,27.56-27.28,41.98-54.24,41.98-20.26,0-32.35-10.13-32.35-27.1,0-19.37,14.63-26.41,38.23-31.5l48.36-10.27ZM1774.35,177.33c-3.17-.52-6.47-.77-10.09-.77-27.5,0-50.92,12.35-62.11,32.47v-24.51c0-3.74-3.04-6.78-6.78-6.78h-40.76c-3.74,0-6.78,3.04-6.78,6.78v217.99c0,3.74,3.04,6.78,6.78,6.78h40.76c3.74,0,6.78-3.04,6.78-6.78v-114.9c0-34.27,23.2-57.3,57.74-57.3,4.7,0,8.63.17,12.74.56,1.89.18,3.79-.45,5.2-1.73,1.41-1.28,2.22-3.11,2.22-5.02v-40.09c0-3.34-2.39-6.16-5.69-6.7ZM1973.75,206.72c-18.32-21.83-44.82-33.36-76.62-33.36s-59.09,12.34-79.33,34.76c-19.98,22.12-30.98,52.52-30.98,85.61,0,70.87,46.26,120.37,112.49,120.37,43.9,0,79.17-21.53,96.77-59.08.8-1.7.85-3.61.14-5.37-.71-1.76-2.14-3.15-3.91-3.82l-33.69-12.76c-3.39-1.28-7.27.37-8.63,3.68-8.08,19.63-26.56,30.9-50.68,30.9-33.08,0-56.1-23.59-60.26-61.64h154.16c3.74,0,6.78-3.04,6.78-6.78v-11.63c0-31.99-9.32-60.72-26.25-80.88ZM1945.22,264.82h-103.37c7.73-28.91,27.66-45.45,54.84-45.45,34.72,0,47.8,23.3,48.52,45.45Z"/>
                                    </svg>
                                </span>
                            &nbsp; and a receipt was sent to the email on the order</p>
                        </div>
                    </div>
                </div>);
            case SquareOrderStatus.COMPLETED: // completed [CURRENTLY UNUSED]
                return (<div id="order-completed"className="product-note-container">
                    <div className="product-note">
                        <div className="title">
                            <i aria-hidden={true} className="fa fa-solid fa-circle-check"></i>
                            <span>Order status: Completed</span>
                        </div>
                        <div className="body">
                            <p className="reason">Your order has been completed.</p>
                        </div>
                    </div>
                </div>);
            default: // proposed, draft -> unfulfilled
                return (<div id="order-confirmation"className="product-note-container">
                    <div className="product-note">
                        <div className="title">
                            <i aria-hidden={true} className="fa fa-solid fa-circle-exclamation"></i>
                            <span>Order status: Unfulfilled</span>
                        </div>
                        <div className="body">
                            <p className="reason">Checkout has not been fulfilled with
                                <span className="square-logo">
                                    <span className="sr-only">Square</span>
                                    <svg aria-hidden={true} focusable={false} aria-label="Square Up" role="img" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 2000 501.43">
                                        <path fill="#1A1A1A" d="M501.43,83.79v333.84c0,46.27-37.5,83.79-83.79,83.79H83.79c-46.28,0-83.79-37.5-83.79-83.79V83.79C0,37.52,37.52,0,83.79,0h333.84c46.29,0,83.79,37.5,83.79,83.79ZM410.23,117.65c0-14.61-11.85-26.45-26.45-26.45H117.63c-14.61,0-26.45,11.84-26.45,26.45v266.19c0,14.61,11.84,26.45,26.45,26.45h266.17c14.61,0,26.45-11.85,26.45-26.45V117.65h-.02ZM182.32,197.6c0-8.43,6.79-15.26,15.17-15.26h106.4c8.39,0,15.17,6.84,15.17,15.26v106.24c0,8.43-6.75,15.26-15.17,15.26h-106.4c-8.39,0-15.17-6.84-15.17-15.26v-106.24ZM778.95,221.94l-3.85-.86c-41.04-9.31-65.81-14.93-65.81-42,0-24.2,23.02-41.11,55.98-41.11,30.52,0,53.74,12.76,73.08,40.16,1.11,1.57,2.84,2.61,4.74,2.84,1.89.23,3.79-.35,5.23-1.59l32.16-27.71c2.68-2.31,3.15-6.22,1.1-9.09-24.19-33.89-67.01-54.12-114.56-54.12-31.56,0-60.34,9.26-81.04,26.08-21.73,17.65-33.21,41.93-33.21,70.23,0,63.76,54.74,76.94,98.71,87.53,4.45,1.08,8.77,2.1,12.95,3.08,39.74,9.36,66,15.54,66,43.74s-24.04,45.48-61.24,45.48c-33.71,0-64.35-17.1-86.28-48.14-1.1-1.55-2.8-2.59-4.68-2.84-1.88-.25-3.73.28-5.2,1.49l-33.86,27.99c-2.72,2.25-3.28,6.14-1.3,9.05,25.63,37.64,76.48,61.97,129.56,61.97,32.56,0,62.52-9.57,84.36-26.95,23.27-18.51,35.57-44.01,35.57-73.73,0-67.27-57.62-80.13-108.45-91.48ZM1126.34,177.74h-40.76c-3.74,0-6.78,3.04-6.78,6.78v19.06c-12.6-14.21-33.77-30.22-65.18-30.22s-56.88,12.32-75.37,35.62c-17.16,21.63-26.62,51.73-26.62,84.75s9.45,63.12,26.62,84.75c18.49,23.31,44.56,35.62,75.37,35.62,26.63,0,49.1-9.45,65.18-27.37v107.92c0,3.74,3.04,6.78,6.78,6.78h40.76c3.74,0,6.78-3.04,6.78-6.78V184.52c0-3.74-3.04-6.78-6.78-6.78ZM1080.11,287.17v13.57c0,39.86-21.97,65.61-55.98,65.61-36.15,0-57.74-27.15-57.74-72.61s21.58-72.61,57.74-72.61c34.01,0,55.98,25.93,55.98,66.05ZM1360.6,177.74h-40.76c-3.74,0-6.78,3.04-6.78,6.78v130.66c0,32.45-23.32,49.42-46.36,49.42-26.03,0-39.79-15.58-39.79-45.04v-135.03c0-3.74-3.04-6.78-6.78-6.78h-40.76c-3.74,0-6.78,3.04-6.78,6.78v146.41c0,50.53,30.93,83.17,78.8,83.17,23.76,0,43.95-9.67,61.67-29.56v17.96c0,3.74,3.04,6.78,6.78,6.78h40.76c3.74,0,6.78-3.04,6.78-6.78v-217.99c0-3.74-3.04-6.78-6.78-6.78ZM1607.93,367.07c-8.79-.24-12.72-4.78-12.72-14.7v-98.9c0-51.66-31.71-80.11-89.3-80.11-44.71,0-80.07,23.24-94.59,62.18-.66,1.78-.52,3.78.39,5.47.93,1.73,2.57,2.98,4.48,3.42l37.83,8.71c3.37.77,6.78-1.14,7.94-4.45,6.74-19.11,20.01-28.01,41.76-28.01,25.53,0,38.48,11.62,38.48,34.54v3.2l-62.73,12.98c-53.04,11.03-77.74,34.25-77.74,73.09s32.22,68.73,78.36,68.73c28.25,0,51.2-8.69,66.46-25.16,9.13,17.73,33.93,26.29,62.3,21.35,3.24-.57,5.6-3.38,5.6-6.69v-28.9c0-3.63-2.93-6.67-6.52-6.77ZM1542.21,300.54v26.89c0,27.56-27.28,41.98-54.24,41.98-20.26,0-32.35-10.13-32.35-27.1,0-19.37,14.63-26.41,38.23-31.5l48.36-10.27ZM1774.35,177.33c-3.17-.52-6.47-.77-10.09-.77-27.5,0-50.92,12.35-62.11,32.47v-24.51c0-3.74-3.04-6.78-6.78-6.78h-40.76c-3.74,0-6.78,3.04-6.78,6.78v217.99c0,3.74,3.04,6.78,6.78,6.78h40.76c3.74,0,6.78-3.04,6.78-6.78v-114.9c0-34.27,23.2-57.3,57.74-57.3,4.7,0,8.63.17,12.74.56,1.89.18,3.79-.45,5.2-1.73,1.41-1.28,2.22-3.11,2.22-5.02v-40.09c0-3.34-2.39-6.16-5.69-6.7ZM1973.75,206.72c-18.32-21.83-44.82-33.36-76.62-33.36s-59.09,12.34-79.33,34.76c-19.98,22.12-30.98,52.52-30.98,85.61,0,70.87,46.26,120.37,112.49,120.37,43.9,0,79.17-21.53,96.77-59.08.8-1.7.85-3.61.14-5.37-.71-1.76-2.14-3.15-3.91-3.82l-33.69-12.76c-3.39-1.28-7.27.37-8.63,3.68-8.08,19.63-26.56,30.9-50.68,30.9-33.08,0-56.1-23.59-60.26-61.64h154.16c3.74,0,6.78-3.04,6.78-6.78v-11.63c0-31.99-9.32-60.72-26.25-80.88ZM1945.22,264.82h-103.37c7.73-28.91,27.66-45.45,54.84-45.45,34.72,0,47.8,23.3,48.52,45.45Z"/>
                                    </svg>
                                </span>
                            </p>
                            <button
                                onClick={(e) => {
                                    e.preventDefault();
                                    if(orderInfo.redirect) orderInfo.redirect();
                                }}
                                className="btn-primary"
                                role='link'
                            >
                                Fulfill with
                                <span className="square-logo">
                                    <span className="sr-only">Square</span>
                                    <svg aria-hidden={true} focusable={false} aria-label="Square Up" role="img" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 2000 501.43">
                                        <path fill="#FFF" d="M501.43,83.79v333.84c0,46.27-37.5,83.79-83.79,83.79H83.79c-46.28,0-83.79-37.5-83.79-83.79V83.79C0,37.52,37.52,0,83.79,0h333.84c46.29,0,83.79,37.5,83.79,83.79ZM410.23,117.65c0-14.61-11.85-26.45-26.45-26.45H117.63c-14.61,0-26.45,11.84-26.45,26.45v266.19c0,14.61,11.84,26.45,26.45,26.45h266.17c14.61,0,26.45-11.85,26.45-26.45V117.65h-.02ZM182.32,197.6c0-8.43,6.79-15.26,15.17-15.26h106.4c8.39,0,15.17,6.84,15.17,15.26v106.24c0,8.43-6.75,15.26-15.17,15.26h-106.4c-8.39,0-15.17-6.84-15.17-15.26v-106.24ZM778.95,221.94l-3.85-.86c-41.04-9.31-65.81-14.93-65.81-42,0-24.2,23.02-41.11,55.98-41.11,30.52,0,53.74,12.76,73.08,40.16,1.11,1.57,2.84,2.61,4.74,2.84,1.89.23,3.79-.35,5.23-1.59l32.16-27.71c2.68-2.31,3.15-6.22,1.1-9.09-24.19-33.89-67.01-54.12-114.56-54.12-31.56,0-60.34,9.26-81.04,26.08-21.73,17.65-33.21,41.93-33.21,70.23,0,63.76,54.74,76.94,98.71,87.53,4.45,1.08,8.77,2.1,12.95,3.08,39.74,9.36,66,15.54,66,43.74s-24.04,45.48-61.24,45.48c-33.71,0-64.35-17.1-86.28-48.14-1.1-1.55-2.8-2.59-4.68-2.84-1.88-.25-3.73.28-5.2,1.49l-33.86,27.99c-2.72,2.25-3.28,6.14-1.3,9.05,25.63,37.64,76.48,61.97,129.56,61.97,32.56,0,62.52-9.57,84.36-26.95,23.27-18.51,35.57-44.01,35.57-73.73,0-67.27-57.62-80.13-108.45-91.48ZM1126.34,177.74h-40.76c-3.74,0-6.78,3.04-6.78,6.78v19.06c-12.6-14.21-33.77-30.22-65.18-30.22s-56.88,12.32-75.37,35.62c-17.16,21.63-26.62,51.73-26.62,84.75s9.45,63.12,26.62,84.75c18.49,23.31,44.56,35.62,75.37,35.62,26.63,0,49.1-9.45,65.18-27.37v107.92c0,3.74,3.04,6.78,6.78,6.78h40.76c3.74,0,6.78-3.04,6.78-6.78V184.52c0-3.74-3.04-6.78-6.78-6.78ZM1080.11,287.17v13.57c0,39.86-21.97,65.61-55.98,65.61-36.15,0-57.74-27.15-57.74-72.61s21.58-72.61,57.74-72.61c34.01,0,55.98,25.93,55.98,66.05ZM1360.6,177.74h-40.76c-3.74,0-6.78,3.04-6.78,6.78v130.66c0,32.45-23.32,49.42-46.36,49.42-26.03,0-39.79-15.58-39.79-45.04v-135.03c0-3.74-3.04-6.78-6.78-6.78h-40.76c-3.74,0-6.78,3.04-6.78,6.78v146.41c0,50.53,30.93,83.17,78.8,83.17,23.76,0,43.95-9.67,61.67-29.56v17.96c0,3.74,3.04,6.78,6.78,6.78h40.76c3.74,0,6.78-3.04,6.78-6.78v-217.99c0-3.74-3.04-6.78-6.78-6.78ZM1607.93,367.07c-8.79-.24-12.72-4.78-12.72-14.7v-98.9c0-51.66-31.71-80.11-89.3-80.11-44.71,0-80.07,23.24-94.59,62.18-.66,1.78-.52,3.78.39,5.47.93,1.73,2.57,2.98,4.48,3.42l37.83,8.71c3.37.77,6.78-1.14,7.94-4.45,6.74-19.11,20.01-28.01,41.76-28.01,25.53,0,38.48,11.62,38.48,34.54v3.2l-62.73,12.98c-53.04,11.03-77.74,34.25-77.74,73.09s32.22,68.73,78.36,68.73c28.25,0,51.2-8.69,66.46-25.16,9.13,17.73,33.93,26.29,62.3,21.35,3.24-.57,5.6-3.38,5.6-6.69v-28.9c0-3.63-2.93-6.67-6.52-6.77ZM1542.21,300.54v26.89c0,27.56-27.28,41.98-54.24,41.98-20.26,0-32.35-10.13-32.35-27.1,0-19.37,14.63-26.41,38.23-31.5l48.36-10.27ZM1774.35,177.33c-3.17-.52-6.47-.77-10.09-.77-27.5,0-50.92,12.35-62.11,32.47v-24.51c0-3.74-3.04-6.78-6.78-6.78h-40.76c-3.74,0-6.78,3.04-6.78,6.78v217.99c0,3.74,3.04,6.78,6.78,6.78h40.76c3.74,0,6.78-3.04,6.78-6.78v-114.9c0-34.27,23.2-57.3,57.74-57.3,4.7,0,8.63.17,12.74.56,1.89.18,3.79-.45,5.2-1.73,1.41-1.28,2.22-3.11,2.22-5.02v-40.09c0-3.34-2.39-6.16-5.69-6.7ZM1973.75,206.72c-18.32-21.83-44.82-33.36-76.62-33.36s-59.09,12.34-79.33,34.76c-19.98,22.12-30.98,52.52-30.98,85.61,0,70.87,46.26,120.37,112.49,120.37,43.9,0,79.17-21.53,96.77-59.08.8-1.7.85-3.61.14-5.37-.71-1.76-2.14-3.15-3.91-3.82l-33.69-12.76c-3.39-1.28-7.27.37-8.63,3.68-8.08,19.63-26.56,30.9-50.68,30.9-33.08,0-56.1-23.59-60.26-61.64h154.16c3.74,0,6.78-3.04,6.78-6.78v-11.63c0-31.99-9.32-60.72-26.25-80.88ZM1945.22,264.82h-103.37c7.73-28.91,27.66-45.45,54.84-45.45,34.72,0,47.8,23.3,48.52,45.45Z"/>
                                    </svg>
                                </span>
                                <span aria-hidden={true} className='redirect'><i aria-hidden={true} className="fa fa-solid fa-arrow-up-right-from-square"></i></span>
                            </button>
                            {copyToClipboard && copyToClipboard}
                        </div>
                    </div>
                </div>);
        }
    }
    else{
        return(<></>);
    }
}