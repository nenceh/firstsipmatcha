// trap focus: https://hidde.blog/using-javascript-to-trap-focus-in-an-element/, https://medium.com/cstech/achieving-focus-trapping-in-a-react-modal-component-3f28f596f35b
...
type Params = {
    type: string,
    primaryBtn: (() => void) | Dispatch<SetStateAction<boolean>>,
    secondaryBtn?: (() => void) | Dispatch<SetStateAction<boolean>>,
}

export default function Modal({ type, primaryBtn }: Params) {
    ...
    useEffect(() => { // focus trap
        const handleTab = (e: KeyboardEvent) => {
            if(modalRef.current && e.key === 'Tab'){
                ...
                const focusableElements = modalElement.querySelectorAll('a[href]:not([disabled]), button:not([disabled]), textarea:not([disabled]), input[type="text"]:not([disabled]), input[type="radio"]:not([disabled]), input[type="checkbox"]:not([disabled]), select:not([disabled])');
        ...
        if(modalRef.current){
            ...
            firstFocus.current?.focus(); // need to help direct focus after initial open
        }

        let ref = null;

        if(modalRef.current) ref = modalRef.current;

        return() => {
            if(ref){
                ref.removeEventListener('keydown', handleTab);
            }
        }
    },[modalRef]);
    ...
    switch(type){
        case 'SQUARE_PAYMENT':
            return (<div className="modal-container" id="square-payment" ref={modalRef}>
                <div
                    className="modal"
                    aria-modal={true}
                    aria-label={'Payments with Square'}
                    role="dialog"
                    tabIndex={-1}
                ...
                >
                    <div className="modal-header">
                        <div className="square-logo">
                            <svg aria-label="Square" role="img" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 2000 501.43">
                        ...
                        <button
                            className='btn-icon'
                            onClick={() => primaryBtn(false)}
                            ref={firstFocus}
                            aria-label="Close dialog"
                            autoFocus
                        >
                            <i aria-hidden={true} className="fa fa-solid fa-xmark"></i>
                        </button>
                    </div>
                    <div className="modal-body">
                        Once an order is placed, you will be given a link to complete the purchase with Square.
                    </div>
                </div>
            </div>);
        default:
